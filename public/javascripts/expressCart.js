...
$(document).ready(function () {
   ...
   $('#checkoutInformation')
      ...
      .on('click', function (e) {
         e.preventDefault();
         if (
            $('#shipping-form').validator('validate').has('.has-error')
               .length === 0
         ) {
   ...
   $(document).on('click', '.add-variant-to-cart', function (e) {
      $.ajax({
         ...
         data: {
            ...
            productQuantity: '1',
            productVariant: $(
               '#productVariant-' + $(this).attr('data-id')
            ).val(),
         },
      })
         .done(function (msg) {
   ...
   $(document).on('click', '#confirmWireAccountModal', function (e) {
      $.ajax({
         method: 'POST',
         url: '/checkout/confirmwiretransfer',
      })
         .done(function (msg) {
            $.ajax({
               method: 'POST',
   ...
   if ($('#blockonomics_div').length > 0) {
      ...
      var endDatebl = new Date(
         new Date().getTime() + 1000 * 60 * timeOutMinutes
      );
      ...
      blSocket.onmessage = function (msg) {
         ...
         if (data.status === 0 || data.status === 1 || data.status === 2) {
            ...
            var orderMessage =
               '<br>View <b><a href="/payment/' + orderid + '">Order</a></b>';
            $('#blockonomics_waiting').html(
               'Payment detected (<b>' +
                  data.value / 1e8 +
                  ' BTC</b>).' +
...
const onFilterChange = (target) => {
   ...
   let values = localStorage.getItem('filterTerms')
      ...
      : {};
   values = values = { ...values, [parent]: target.id };

   //const filterTerms = getFilterTerms(values);
   localStorage.setItem('filterTerms', JSON.stringify(values));
