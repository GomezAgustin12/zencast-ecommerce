...
$(document).ready(function () {
   ...
   $('#settingsForm')
      ...
      .on('submit', function (e) {
         if (!e.isDefaultPrevented()) {
            ...
            $('#footerHtml_input').val(
               $('.CodeMirror')[0].CodeMirror.getValue()
            );
            $('#googleAnalytics_input').val(
               $('.CodeMirror')[1].CodeMirror.getValue()
            );
            $('#customCss_input').val(
               $('.CodeMirror')[2].CodeMirror.getValue()
            );
            $.ajax({
               method: 'POST',
         ...
         if (!e.isDefaultPrevented()) {
            ...
            $.ajax({
               ...
               data: {
                  ...
                  productCondition: $('#productCondition').val(),
                  productHashrate: $('#productHashrate').val(),
                  productAlgorithm: $('#productAlgorithm').val(),
                  productWeight: $('#productWeight').val(),
                  productElectricityUsage: $('#productElectricityUsage').val(),
               },
            })
               .done(function (msg) {
                  showNotification(
                     ...
                     'success',
                     false,
                     '/admin/product/edit/' + msg.productId
                  );
               })
               .fail(function (msg) {
                  if (msg.responseJSON.length > 0) {
   ...
   $(document).on('click', '#btn_product_filter', function (e) {
      if ($('#product_filter').val() !== '') {
         window.location.href =
            '/admin/product/filter/' + $('#product_filter').val();
      } else {
   ...
   $(document).on('click', '#btn_order_filter', function (e) {
      if ($('#order_filter').val() !== '') {
         window.location.href =
            '/admin/order/filter/' + $('#order_filter').val();
      } else {
         showNotification('Please enter a keyword to filter', 'danger');
      }
   });
   ...
   $(document).on('click', '#btn_customer_filter', function (e) {
      if ($('#customer_filter').val() !== '') {
         window.location.href =
            '/admin/customer/filter/' + $('#customer_filter').val();
      } else {
         showNotification('Please enter a keyword to filter', 'danger');
      }
   });
   ...
   $(document).on('click', '#btn_review_filter', function (e) {
      if ($('#review_filter').val() !== '') {
         window.location.href =
            '/admin/review/filter/' + $('#review_filter').val();
      } else {
         showNotification('Please enter a keyword to filter', 'danger');
      }
   });
   ...
   $(document).on('click', '#orderCreate', function (e) {
      e.preventDefault();
      if (
         $('#createOrderForm').validator('validate').has('.has-error')
            .length === 0
      ) {
         $.ajax({
            method: 'POST',
   ...
   $(document).on('click', '.orderFilterByStatus', function (e) {
      e.preventDefault();
      window.location =
         '/admin/order/bystatus/' + $('#orderStatusFilter').val();
   });
   ...
   $('#deleteCustomer').on('click', function (e) {
      ...
      $.ajax({
         ...
         .done(function (msg) {
            showNotification(msg.message, 'success', false, '/admin/customer');
         })
         .fail(function (msg) {
            showNotification(msg.responseJSON.message, 'danger');
         });
   ...
   if ($('#footerHtml').length) {
      var footerHTML = window.CodeMirror.fromTextArea(
   ...
   if ($('#customCss').length) {
      var customCss = window.CodeMirror.fromTextArea(
...
function globalSearch() {
   ...
   $.ajax({
      ...
      res.products.forEach((value) => {
         hasResult = true;
         const result = `<li class="list-group-item global-result text-center" data-url="/admin/product/edit/${
            value._id
         }">
                <div class="row">
...
function validationErrors(errors) {
   ...
   errors.forEach((value) => {
      errorMessage += `<p>${value.dataPath.replace(
         '/',
         ''
      )} - <span class="text-danger">${value.message}<span></p>`;
   });
   return errorMessage;
}
