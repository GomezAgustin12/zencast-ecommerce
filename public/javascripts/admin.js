...
$(document).ready(function () {
  ...
  $(document).on("click", "#btn_order_filter", function (e) {
    if ($("#order_filter").val() !== "") {
      window.location.href = "/admin/order/filter/" + $("#order_filter").val();
    } else {
      showNotification("Please enter a keyword to filter", "danger");
    }
  });
  ...
  $(document).on("click", "#orderCreate", function (e) {
    e.preventDefault();
    if (
      $("#createOrderForm").validator("validate").has(".has-error").length === 0
    ) {
      $.ajax({
        method: "POST",
  ...
  $(document).on("click", ".orderFilterByStatus", function (e) {
    e.preventDefault();
    window.location = "/admin/order/bystatus/" + $("#orderStatusFilter").val();
  });
  ...
  $("#updateCustomer")
    ...
    .on("click", function (e) {
      e.preventDefault();
      if (
        $("#customer-form").validator("validate").has(".has-error").length === 0
      ) {
        $.ajax({
          method: "POST",
