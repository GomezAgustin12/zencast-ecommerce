...
const testData = fs.readFileSync(
  path.join(__dirname, "..", "bin", "testdata.json"),
  "utf-8"
);
    ...
    .then(() => {
      Promise.all([
        ...
        .then(async () => {
          ...
          await db.variants.insertMany(
            await fixProductIds(jsonData.variants, products)
          );
          await db.reviews.insertMany(
            await fixReviews(jsonData.reviews, products, customers)
          );
