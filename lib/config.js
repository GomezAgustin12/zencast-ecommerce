...
const getConfig = () => {
   ...
   if (fs.existsSync(localConfigFilePath)) {
      const localConfigFile = JSON.parse(
         fs.readFileSync(localConfigFilePath, 'utf8')
      );
   ...
   try {
      ...
      Object.keys(config.modules.enabled).forEach((mod) => {
         config.modules.loaded[
            mod
         ] = require(`./modules/${config.modules.enabled[mod]}`);
      });
   } catch (ex) {
...
const getPaymentConfig = (gateway) => {
   ...
   _.forEach(siteConfig.paymentGateway, (gateway) => {
      const gateConfigFile = path.join(
         __dirname,
         '../config',
         'payment',
         'config',
...
const updateConfig = (fields) => {
   ...
   if (fs.existsSync(localSettingsFile)) {
      ...
      try {
         fs.writeFileSync(
            localSettingsFile,
            JSON.stringify(localSettings, null, 4)
         );
      } catch (exception) {
