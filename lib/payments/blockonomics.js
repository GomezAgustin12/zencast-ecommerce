        ...
        try {
          await db.orders.updateOne(
            ...
            },
            {
              $set: {
                orderStatus: "Paid",
                orderReceivedBtc: amount,
                orderBlockonomicsTxid: txid,
              },
            },
            { multi: false }
          );
...
router.post("/checkout_action", (req, res, next) => {
  ...
  axios
    .get(
      blockonomicsConfig.hostUrl +
        blockonomicsConfig.priceApi +
        config.currencyISO
    )
    .then((response) => {
      blockonomicsParams.expectedBtc =
        Math.round(
          (req.session.totalCartAmount / response.data.price) * Math.pow(10, 8)
        ) / Math.pow(10, 8);
      // get new address
      axios
        .post(
