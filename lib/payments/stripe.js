...
router.post('/checkout_action', async (req, res, next) => {
   ...
   const chargePayload = {
      amount: numeral(req.session.totalCartAmount)
         .format('0.00')
         .replace('.', ''),
   ...
   stripe.charges.create(chargePayload, (err, charge) => {
      ...
      const orderDoc = {
         ...
         orderTranckingNumber: randomNumber(),
...
router.all('/subscription_update', async (req, res, next) => {
   ...
   if (stripeSigSecret) {
      try {
         hook = await stripe.webhooks.constructEvent(
            req.rawBody,
            stripeSig,
            stripeSigSecret
         );
...
router.post('/checkout_action_subscription', async (req, res, next) => {
   ...
   try {
      ...
      if (!plan) {
         req.session.messageType = 'danger';
         req.session.message =
            "The plan connected to this product doesn't exist";
         res.redirect('/checkout/payment');
         return;
      }
   } catch (ex) {
