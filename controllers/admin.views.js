...
const { UserRepo, ProductRepo, OrdersRepo, VariantsRepo } = require('../repositories');
...
const adminViews = {
	...
	filter: async (req, res, next) => {
		...
		const results = await ProductRepo.findMany({ query: { _id: { $in: lunrIdArray } } });
	...
	editProduct: async (req, res) => {
		...
		const product = await ProductRepo.findOne({ _id: getId(req.params.id) });
		if (!product) {
		...
		// Get variants
		product.variants = await VariantsRepo.findMany({ query: { product: getId(req.params.id) } });
