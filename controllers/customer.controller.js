...
const customerCtrl = {
	create: async (req, res) => {
		try {
			...
			const customer = await CustomerRepo.findOne({ email: customerObj.email });
			if (customer) throw Error('A customer already exists with that email address');
	...
	update: async function (req, res) {
		try {
			...
			console.error('🔥🔥', colors.red(`Failed updating customer: ${error} \n ${error.stack}`));
	...
	delete: async (req, res) => {
		try {
			...
			console.error('🔥🔥', colors.red(`Failed deleting customer: ${error}`));
			res.status(400).json({ message: `Failed deleting customer: ${error.message}` });
		}
	},
	findByEmail: async (req, res) => {
		try {
			...
			const customer = await CustomerRepo.findOne({ email: customerEmail });
			...
			res.status(400).json({ message: `Failed to find customer: ${error.message}` });
		}
	},
	login: async (req, res) => {
