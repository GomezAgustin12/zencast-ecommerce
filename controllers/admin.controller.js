...
const adminCtrl = {
	login: async (req, res) => {
		const user = await UserRepo.findOne({ userEmail: mongoSanitize(req.body.email) });
		if (!user || user === null) {
	...
	create: async (req, res) => {
		try {
			...
			console.error('🔥🔥', colors.red(`Error inserting document: ${error}`));
	...
	addVariant: async (req, res) => {
		try {
			...
			const product = await ProductRepo.findOne({ _id: getId(req.body.product) });
	...
	editVariant: async (req, res) => {
		...
		const variant = await VariantsRepo.findOne({ _id: getId(req.body.variant) });
		if (!variant) {
			res.status(400).json({ message: 'Failed to add product variant' });
			return;
		}
	...
	updateProduct: async (req, res) => {
		const product = await ProductRepo.findOne({ _id: getId(req.body.productId) });
	...
	deleteImage: async (req, res) => {
		...
		const product = await ProductRepo.findOne({ _id: getId(req.body.product_id) });
		if (!product) {
