...
const { OrdersRepo, ProductRepo, VariantsRepo, CartRepo, PagesRepo } = require('../repositories');
...
router.get('/payment/:orderId', async (req, res, next) => {
	...
	if (config.trackStock && req.session.paymentApproved) {
		...
		if (order.productStockUpdated !== true) {
			Object.keys(order.orderProducts).forEach(async (productKey) => {
				...
				const dbProduct = await ProductRepo.findOne({ _id: getId(product.productId) });
	...
	if (config.orderHook && !order.hookSent) {
		await hooker(order);
		await OrdersRepo.updateOne({ query: { _id: getId(order._id) }, set: { hookSent: true } });
	}
...
router.get('/:page?', async (req, res, next) => {
	...
	if (!req.params.page) {
		...
		const page = await PagesRepo.findOne({ pageSlug: req.params.page, pageEnabled: 'true' });
