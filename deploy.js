...
const compressJS = async () => {
   const publicFiles = globby.sync(
      ['public/javascripts/*.js', '!public/javascripts/*.min.js'],
      {
         nosort: true,
      }
   );
   ...
   publicFiles.forEach(async (file) => {
      ...
      fs.writeFileSync(
         `${parseFilePath.dir}/${parseFilePath.name}.min.js`,
         minified.code
      );
   });

   const themeFiles = globby.sync(
      ['views/themes/**/*.js', '!views/themes/**/*.min.js'],
      {
         nosort: true,
      }
   );
...
const compressCss = async () => {
   ...
   const publicFiles = globby.sync(['public/stylesheets/less/*.less'], {
      nosort: true,
   });
   ...
   publicFiles.forEach(async (file) => {
      ...
      // Write less style
      fs.writeFileSync(
         `${publicOutputPath}/${parseFilePath.name}.css`,
         less.css
      );
      ...
      // Write minified css
      fs.writeFileSync(
         `${publicOutputPath}/${parseFilePath.name}.min.css`,
         minified
      );
   });

   const themeFiles = globby.sync(['views/themes/*.less'], { nosort: true });
   themeFiles.forEach(async (file) => {
      const parseFilePath = path.parse(file);
      // Process the less
      const less = await render(fs.readFileSync(file, 'utf-8'), {});
      ...
      // Write less style
      fs.writeFileSync(
         `${themeOutputPath}/${parseFilePath.name}.css`,
         less.css
      );
      ...
      // Write minified css
      fs.writeFileSync(
         `${themeOutputPath}/${parseFilePath.name}.min.css`,
         minified
      );
   });
};
